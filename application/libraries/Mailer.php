...
class Mailer {
    ...
    public function send_with_attachment($data){
        ...
        $mail->Host = 'mxgate.p2k.co.id';
        $mail->Username = 'support@edunitas.com'; // Email Pengirim
        $mail->Password = 'support2021'; // Isikan dengan Password email pengirim
        $mail->Port = 2525;
        $mail->SMTPAuth = true;
        $mail->SMTPSecure = 'ssl';
        $mail->SMTPDebug = 2; // Aktifkan untuk melakukan debugging

        $mail->setFrom('support@edunitas.com', 'cek');
        $mail->addAddress($data['email_penerima'], '');
        $mail->isHTML(true); // Aktifkan jika isi emailnya berupa html
